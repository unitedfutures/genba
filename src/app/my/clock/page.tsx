...
export default function ClockPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: log }, { data: siteList }] = await Promise.all([
    ...
    if (log) {
      setTodayLog(log)
      setState(log.clock_out_at ? 'finished' : 'working')
      if (log.site_id) setSelectedSite(log.site_id)
    }
    setLoading(false)
  }, [today])
      ...
      {state === 'working' && todayLog && (
        <div className="space-y-4">
          <div className="card bg-blue-50 border-blue-100">
            ...
            <p className="text-sm text-blue-600 mt-1">{(todayLog.site as any)?.name}</p>
